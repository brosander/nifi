...
public class NifiPropertiesHelperTest {
    @Test
    public void testCanUpdateAllKeys() throws IllegalAccessException, IOException {
        String testValue = NifiPropertiesHelperTest.class.getCanonicalName();

        // Get all property keys from NiFiProperties
        ...
        for (Field field : NiFiProperties.class.getDeclaredFields()) {
            ...
            if (Modifier.isStatic(modifiers) && Modifier.isPublic(modifiers) && field.getType() == String.class) {
                if (!field.getName().toLowerCase().startsWith("default")) {
                    propertyKeys.add((String) field.get(null));
                }
            }
        }
        ...
        // Set them all to testValue with the helper
        new NifiPropertiesHelper().outputWithUpdatedPropertyValues(outputStream, propertyKeys.stream().collect(Collectors.toMap(s -> s, s -> testValue)));
