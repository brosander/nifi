...
public class TlsToolkitMainTest {
    ...
    public void testBadParse() {
        runAndAssertExitCode(SSLToolkitCommandLine.ERROR_PARSING_COMMAND_LINE, "--unknownArgument");
    }

    @Test
    public void testHelp() {
        runAndAssertExitCode(SSLToolkitCommandLine.HELP_EXIT_CODE, "-h");
        runAndAssertExitCode(SSLToolkitCommandLine.HELP_EXIT_CODE, "--help");
    }

    @Test
    public void testDirOutput() throws Exception {
        runAndAssertExitCode(0, "-o", tempDir.getAbsolutePath());
        X509Certificate x509Certificate = checkLoadCertPrivateKey(SSLToolkitCommandLine.DEFAULT_KEY_ALGORITHM);

        Properties nifiProperties = checkHostDirAndReturnNifiProperties(SSLToolkitCommandLine.DEFAULT_HOSTNAMES, x509Certificate);
