...
public abstract class BaseCommandLine {
    ...
    public static final OptionWithDefault CERTIFICATE_AUTHORITY_HOSTNAME_OPTION = new OptionWithDefault("c", CERTIFICATE_AUTHORITY_HOSTNAME_ARG, "Hostname of NiFi Certificate Authority", TlsConfig.DEFAULT_HOSTNAME);
    public static final OptionWithDefault DAYS_OPTION = new OptionWithDefault("d", DAYS_ARG, "Number of days issued certificate should be valid for.", TlsConfig.DEFAULT_DAYS);
    public static final OptionWithDefault KEY_ALGORITHM_OPTION = new OptionWithDefault("a", KEY_ALGORITHM_ARG, "Algorithm to use for generated keys.", TlsConfig.DEFAULT_KEY_PAIR_ALGORITHM);
    public static final OptionWithDefault KEY_SIZE_OPTION = new OptionWithDefault("k", KEY_SIZE_ARG, "Number of bits for generated keys.", TlsConfig.DEFAULT_KEY_SIZE);
    public static final OptionWithDefault KEY_STORE_TYPE_OPTION = new OptionWithDefault("T", KEY_STORE_TYPE_ARG, "The type of keyStores to generate.", TlsConfig.DEFAULT_KEY_STORE_TYPE);
    public static final OptionWithDefault SIGNING_ALGORITHM_OPTION = new OptionWithDefault("s", SIGNING_ALGORITHM_ARG, "Algorithm to use for signing certificates.", TlsConfig.DEFAULT_SIGNING_ALGORITHM);

    public static final Option DIFFERENT_KEY_AND_KEYSTORE_PASSWORDS_OPTION = new Option("g", DIFFERENT_KEY_AND_KEYSTORE_PASSWORDS_ARG, false, "Use different generated password for the key and the keyStore.");
    public static final Option HELP_OPTION = new Option("h", HELP_ARG, false, "Print help and exit.");
    ...
    public BaseCommandLine(String header) {
        ...
        addOption(KEY_STORE_TYPE_OPTION.getOption());
        addOption(HELP_OPTION);
        addOption(CERTIFICATE_AUTHORITY_HOSTNAME_OPTION.getOption());
        addOption(KEY_ALGORITHM_OPTION.getOption());
        addOption(KEY_SIZE_OPTION.getOption());
        addOption(DIFFERENT_KEY_AND_KEYSTORE_PASSWORDS_OPTION);
    }
    ...
    protected void addOption(Option option) {
        if (options.hasShortOption(option.getOpt())) {
            throw new IllegalStateException("Already have option for -" + option.getOpt());
        }
        if (options.hasLongOption(option.getLongOpt())) {
            throw new IllegalStateException("Already have option for --" + option.getLongOpt());
        }
    ...
    protected CommandLine doParse(String[] args) throws CommandLineParseException {
        ...
        try {
            ...
            certificateAuthorityHostname = CERTIFICATE_AUTHORITY_HOSTNAME_OPTION.getValue(commandLine);
            days = DAYS_OPTION.getIntValue(commandLine);
            keySize = KEY_SIZE_OPTION.getIntValue(commandLine);
            keyAlgorithm = KEY_ALGORITHM_OPTION.getValue(commandLine);
            keyStoreType = KEY_STORE_TYPE_OPTION.getValue(commandLine);
            signingAlgorithm = SIGNING_ALGORITHM_OPTION.getValue(commandLine);
