 ...
 */
public class NifiPropertiesConfigurationWriter implements ConfigurationWriter<CertificateAuthorityClientConfig> {
    ...
    public NifiPropertiesConfigurationWriter(NiFiPropertiesWriterFactory niFiPropertiesWriterFactory, String hostname, int hostNum) throws IOException {
    ...
    @Override
    public void write(CertificateAuthorityClientConfig tlsClientConfig, File baseDir, OutputStreamFactory outputStreamFactory) throws IOException {
        ...
        try (OutputStream stream = outputStreamFactory.create(new File(baseDir, "nifi.properties"))) {
            niFiPropertiesWriter.writeNiFiProperties(stream);
        }
    }

    protected void updateProperties(NiFiPropertiesWriter niFiPropertiesWriter, CertificateAuthorityClientConfig tlsClientConfig) throws IOException {
        KeyStoreConfig keyStoreConfig = tlsClientConfig.getKeyStoreConfig();
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_KEYSTORE, CONF + new File(keyStoreConfig.getKeyStore()).getName());
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_KEYSTORE_TYPE, keyStoreConfig.getKeyStoreType());
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_KEYSTORE_PASSWD, keyStoreConfig.getKeyStorePassword());
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_KEY_PASSWD, keyStoreConfig.getKeyPassword());

        KeyStoreConfig trustStoreConfig = tlsClientConfig.getTrustStoreConfig();
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_TRUSTSTORE, CONF + new File(trustStoreConfig.getKeyStore()).getName());
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_TRUSTSTORE_TYPE, trustStoreConfig.getKeyStoreType());
        niFiPropertiesWriter.setPropertyValue(NiFiProperties.SECURITY_TRUSTSTORE_PASSWD, trustStoreConfig.getKeyStorePassword());
